 ...
 * @return {Promise}
 */
const imageMin = (argv, inputPath, outputPath) => {
	return file.getSubDirs(inputPath)
		.then(paths => Promise.all(paths.map(input => {
			const sRealInputPath = handleImagePath(input);
			const sRealOutputPath = input.replace(inputPath, outputPath);
...
module.exports = (argv, isModule) => {
	...
	if (file.isFile(sInputPath)) {
		if (argv.o) {
			...
			sOutputPath = file.realpath(path.dirname(sInputPath));
		}
	}
	...
	if (isModule) {
		return imageMin(argv, sInputPath, sOutputPath)
			.then(files => {
