...
const imageMin = (argv) => {
	...
	file.computDirSize(input_path)
		.then(size => {
			colors.warning('待压缩目录 ' + input_path + ' (size: ' + size + ' 字节) 压缩中 ...');
		...
		.then(files => {
			colors.warning('\n图片压缩至：');
			console.log([].concat(...files).map(item => item.path).join('\n'));
			colors.success('共 ' + newFiles.length + ' 张图片压缩完成');
			return file.computDirSize(out_path);
		})
		.then((size) => {
			colors.warning('\n输出目录 ' + out_path + ' (size: ' + size + ' 字节)');
		})
		.catch((e) => {
			colors.error(e);
		});
