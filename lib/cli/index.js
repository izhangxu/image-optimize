...
module.exports = (argv, callback) => {
	...
	input_path = relativePath.exec(_input) ? path.join(process.cwd(), _input) : _input;
	...
	file.computDirSize(input_path)
		.then(size => {
			!callback && colors.warning('待压缩目录 ' + input_path + ' 图片共(size: ' + Math.round(size / 1024) + ' k) 压缩中 ...');
		...
		.then(stats => {
			if (!callback) {
				...
				colors.warning('\n输出目录 ' + out_path + ' 压缩完成的图片共(size: ' + Math.round(countSize / 1024) + ' K)');
			}
		})
		.catch((e) => {
			if (callback) {
