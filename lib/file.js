			...
			.then(stats => {
				...
				if (stats.isDirectory()) {
					return pfs.readdir(dir)
						.then(list => Promise.all(list.map(item => file.listDir(path.resolve(dir, item)))))
		...
		return file.listFiles(dir)
			.then(file => {
				Promise.all(file.map(item => {
						return pify(fs).stat(item);
					}))
					.then(stat => {
						console.log(stat);
						stat.reduce((sum, value) => {
							return sum + value;
						});
					});
			});
